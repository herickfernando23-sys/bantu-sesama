...
async function main() {
  try {
    ...
    console.log('Setting Campaigns sequence so next id will be 1 for table:', tableName);

    // Set sequence value to 1 and mark as not called so nextval returns 1
    await sequelize.query(`SELECT setval(pg_get_serial_sequence('"${tableName}"','id'), 1, false)`);
